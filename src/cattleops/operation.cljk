(ns cattleops.operation
  "OperationActor -- one cattle-raising operation = one supervised actor
  run. The advisor (CattleOpsAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Ranching
  ...
  One run = one cattle-raising coordination operation (intake -> advise
  -> govern -> decide -> commit | hold | approval). No unbounded inner
  loop -- each operation is auditable and checkpointed. A facility's
  operating history is advanced by MANY operations (log-herd-record /
  ...
  hands the decision to a human operator (rancher / veterinarian). The
  approver resumes the pending request once a decision is made.
  `:flag-animal-health-concern` ALWAYS reaches escalation when the
  Governor is clean -- see `cattleops.governor/always-escalate-ops`.

  NOTE: langgraph-clj StateGraph integration is deferred (mirrors
  `meatprocessing.operation`, cloud-itonami-isic-1010). This stub version
  ...
  (:require [cattleops.advisor :as advisor]
...
(defn run-operation
  "Run one cattle-raising operation through the advisor -> governor ->
...
(defn build
  ...
  run-operation for testing. opts:
    :advisor -- a `cattleops.advisor/Advisor` (default: mock-advisor)"
  [store & [opts]]
