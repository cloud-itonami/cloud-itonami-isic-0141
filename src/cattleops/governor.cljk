(ns cattleops.governor
  "Ranching Operations Governor -- the independent compliance layer that
  earns the CattleOpsAdvisor the right to commit. The LLM has no notion
  of:
  ...
  categorically outside its authority (rancher/veterinarian exclusive).
  ...
  escalates to a human (rancher/veterinarian) for final sign-off. The
  LLM's confidence is never sufficient for animal-welfare decisions.
  ...
  This design mirrors `meatprocessing.governor` (cloud-itonami-isic-1010)
  but specializes ranch back-office coordination concerns (facility
  registration, closed op allowlist, treatment/slaughter exclusion, cost
  threshold) rather than food-safety/cold-chain concerns."
  (:require [cattleops.facts :as facts]
...
(defn- cost-threshold-for
  ...
  category-specific threshold from `cattleops.facts` if the category is
...
(defn check
  "Censors a CattleOpsAdvisor proposal against the Governor rules.
