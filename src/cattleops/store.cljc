(ns cattleops.store
  "Store abstraction for cattle-raising facility/herd records. Current
  ...
  use). Mirrors `meatprocessing.store` (cloud-itonami-isic-1010) in shape.

  A registered facility is the minimal unit of authority: a ranch/farm
  operation must be registered before ANY proposal referencing it can be
  considered by the Governor (see `cattleops.governor`'s
  `facility-registered` invariant). Facility data is opaque to this
  namespace -- callers/backends decide what a facility record contains
  (name, location, herd roster, etc); this Store only answers \"is this
  facility-id registered, and if so what's on file\".")
