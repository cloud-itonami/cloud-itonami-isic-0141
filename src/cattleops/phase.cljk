(ns cattleops.phase
  "Phase gate for the cattle-raising operations coordinator rollout
  (0->3 maturity). Mirrors `meatprocessing.phase` (cloud-itonami-isic-1010)
  in shape, adapted to this actor's simpler op set (no separate
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`cattleops.governor/always-escalate-ops`) escalate even when clean;
  ...
  (:require [cattleops.governor :as governor]))
