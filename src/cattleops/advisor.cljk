(ns cattleops.advisor
  "CattleOpsAdvisor -- the contained LLM/decision node. This actor's
  ...
  `meatprocessing.advisor`, cloud-itonami-isic-1010)."
  )
    ...
    (let [{:keys [op facility-id]} request]
      (case op
        ...
        {:op :log-herd-record
         ...
         :summary "Herd count/weight/health-check entry logged from operator submission"
         :confidence 0.9}
        ...
        {:op :flag-animal-health-concern
         ...
         :summary "Animal health/welfare concern flagged for veterinary/rancher review"
         :confidence 0.8}
        ...
        {:op :order-supplies
         ...
         :summary "Supply order (feed/veterinary-supply/equipment) proposed for facility"
         :confidence 0.85}
